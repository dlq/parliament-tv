...
- Started TestFlight/App Store readiness: bundle ID now uses the owned `dlq.ca` domain as `ca.dlq.parliaments`, platform support is scoped to iPhone, iPad, macOS, and Apple TV, and `TESTFLIGHT.md` tracks beta notes, review notes, privacy summary, and local release checks. Still review signing, provisioning, app sandbox, privacy nutrition, support URL, archive uploads, and platform-specific assets.

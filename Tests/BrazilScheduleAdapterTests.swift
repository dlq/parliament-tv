...
final class BrazilScheduleAdapterTests: XCTestCase {
    func testBuildsNowNextMetadataFromWeeklyProgrammingRows() throws {
        ...
        let metadata = try XCTUnwrap(BrazilScheduleAdapter.programMetadata(
            from: html,
            now: now,
            calendar: Calendar(identifier: .gregorian),
            sourceTimeZone: TimeZone(identifier: "America/Sao_Paulo")!,
            displayTimeZone: TimeZone(identifier: "America/Toronto")!
        ))

        XCTAssertEqual(metadata.currentEventTitle, "Participação Popular: Os desafios das relações na Era Digital")
        XCTAssertEqual(metadata.currentEventTime, "12:00 PM - 1:00 PM ET")
        XCTAssertEqual(metadata.nextEventTitle, "Câmara Debate: Regulamentação da Inteligência Artificial")

...
enum BrazilScheduleAdapter {
  ...
  nonisolated static let scheduleURL = URL(
    string: "https://www.camara.leg.br/tv/programacao-semanal")!
  ...
  nonisolated static func programMetadata(
    ...
    let entries = scheduleEntries(
      from: html, now: now, calendar: calendar, sourceTimeZone: sourceTimeZone)
    ...
    if let nextIndex {
      currentIndex =
        nextIndex == entries.startIndex ? entries.startIndex : entries.index(before: nextIndex)
    } else {
    ...
    return ProgramMetadata(
      ...
      currentEventTime: ScheduleTextHelpers.timeRange(
        start: current.start, end: next?.start, timeZone: displayTimeZone),
  ...
  private nonisolated static func scheduleEntries(
    ...
    let pattern =
      #"<tr[^>]*>\s*<td[^>]*>\s*<span[^>]*>(\d{1,2}:\d{2})</span>\s*</td>\s*<td[^>]*>\s*<span[^>]*>([\s\S]*?)</span>\s*</td>\s*</tr>"#
  ...
  private nonisolated static func activeTabHTML(from html: String) -> String? {
    let pattern =
      #"<div[^>]*class="[^"]*\btab-pane\b[^"]*\bactive\b[^"]*"[^>]*>([\s\S]*?)(?=<div[^>]*class="[^"]*\btab-pane\b|</div>\s*</div>\s*</div>)"#

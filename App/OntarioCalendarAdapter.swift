...
enum OntarioCalendarAdapter {
    nonisolated static let calendarURL = URL(string: "https://www.ola.org/en/legislative-business/calendar")!
    ...
    nonisolated static func programMetadataByChannelID(
        ...
        guard !events.isEmpty else {
            if html.range(of: "There are no events today", options: .caseInsensitive) != nil {
                let metadata = ProgramMetadata(
                    ...
                    currentEventTime: "Checked \(ScheduleTextHelpers.timeLabel(for: now, timeZone: displayTimeZone))",
                    nextEventTitle: nil,
                    nextEventTime: nil,
        ...
        if let nextIndex {
            currentIndex = nextIndex == events.startIndex ? events.startIndex : events.index(before: nextIndex)
        } else {
        ...
        let metadata = ProgramMetadata(
            ...
            currentEventTime: ScheduleTextHelpers.timeLabel(for: current.start, timeZone: displayTimeZone),
    ...
    private nonisolated static func calendarEvents(from html: String) -> [CalendarEvent] {
        let pattern = #"<time[^>]*datetime="([^"]+)"[^>]*>[\s\S]*?</time>[\s\S]*?<h[1-6][^>]*>([\s\S]*?)</h[1-6]>"#
